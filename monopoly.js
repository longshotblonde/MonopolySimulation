...
function Player(name, color, money) {
	this.name = name;
	this.color = color;
	this.position = 0;
	this.money = money;
	...
	$("#popupbackground").fadeIn(400, function() {
        ...
        setTimeout(function(){
            if(!timeoutRunning){
                ...
                setTimeout(function(){
                    $("#popupclose").click();
                    console.log("clicked popupclose");
                    timeoutRunning = false;
                }, 1000);
            }
        }, 1000);
...
function customBuy(turn, propIndex){
    var p = player[turn];
    var prop = square[propIndex];
    prop.owner = turn;
    p.pay(0,0);
    updateMoney();
    updateOwned();
...
function setup() {
	...
	for (var i = 1; i <= pcount; i++) {
        ...
        p.money = document.getElementById("player" + i + "startmoney").value.toLowerCase();
    ...
    $("#player1input input:checkbox:checked").each(function() {
        player[turn+1].name == "AI Test 1" ? customBuy(player[turn+1].index, $(this).val()) : customBuy(player[turn+2].index, $(this).val());
    });

    $("#player2input input:checkbox:checked").each(function() {
        player[turn+1].name == "AI Test 2" ? customBuy(player[turn+1].index, $(this).val()) : customBuy(player[turn+2].index, $(this).val());
    });
